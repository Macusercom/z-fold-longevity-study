...
The aggregated data the page reads from `docs/stats.json` is generated by:

```bash
python3 Debug/analyze.py
```

That script applies the filters above, computes every per-generation, per-cohort and per-protector breakdown, and writes the JSON the page consumes. It is the single source of truth — change the filters there and rebuild.
...
docs/                       Static site (GitHub Pages root)
  ...
  imprint.html              Legal notice (Austrian imprint)
  og-image.svg              Social card
  sitemap.xml / robots.txt  SEO

Debug/                      Raw inputs & analysis script
  Samsung Z Fold Questionnaire (Antworten).csv
  analyze.py
  Study 1.txt               2024 study writeup
  Study 2.txt               2025 study writeup
  imprint.html              Source imprint text
  privacy.html              Source privacy text
```

## Prior studies

- [2024 results on r/GalaxyFold](https://old.reddit.com/r/GalaxyFold/comments/1c7vflv/results_study_about_the_z_fold_series/)
- 2025 follow-up (see `Debug/Study 2.txt`)
